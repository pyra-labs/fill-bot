...
export const filterOrdersForMissed = <T extends TimeLocked>(
	...
	return orders.filter(
		(order) => order.account.timeLock.releaseSlot.toNumber() < oneMinuteAgo,
	);
};
...
export const hasAta = async (
	...
	const ataInfo = await retryWithBackoff(() => connection.getAccountInfo(ata));
	return ataInfo !== null;
};
...
export async function getComputerUnitLimitIxMinCu(
	...
	const computeUnitLimit = await getComputeUnitLimitMinCu(
		connection,
		instructions,
		address,
		blockhash,
	...
	return {
		ix: ComputeBudgetProgram.setComputeUnitLimit({
...
export async function buildTransactionMinCU(
	...
	const { ix: ix_computeLimit, limit: computeUnitLimit } =
		await getComputerUnitLimitIxMinCu(
			connection,
			instructions,
			payer,
			blockhash,
	...
	const gasFeeLamports = baseGasLamports + gasMicroLamports / 1_000_000;
