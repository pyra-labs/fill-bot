...
export class FillBot extends AppLogger {
	...
	private isExpectedSimulationError(simulation: SimulationResult): boolean {
		...
		return FillBot.EXPECTED_SIMULATION_ERRORS.some((expectedError) =>
	...
	private buildSendAndConfirm = async (
		...
		return await retryWithBackoff(async () => {
			...
			if (simulation.simulationError) {
				...
				if (this.isExpectedSimulationError(simulation)) {
					this.logger.warn(
						`Simulation failed with expected error, skipping: ${simulation.simulationError}`,
					);
					return null;
				}
